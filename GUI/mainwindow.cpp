...
void MainWindow::createActions()
{
    ...
    action_start_recording_VTK->setEnabled(true);
    action_stop_recording_VTK->setEnabled(false);
...
//			LOG_QMSG(label_str);
    ...
    connect(action_start_recording_VTK, SIGNAL(triggered()), this, SLOT(startRecorderVTK()));
    connect(action_stop_recording_VTK, SIGNAL(triggered()), this, SLOT(stopRecorderVTK()));
...
//--------------------------------------------------------------------------------------------------------
// Obtain the list of components, create UI entries.
// In this case all the default values are stored in the .xml files, and they will not be included in
// params.cpp
//--------------------------------------------------------------------------------------------------------
void MainWindow:: setupCellML()
{
    ...
    int nvariables, ncomponents;
    QLineEdit *line;

    LOG_MSG("setupCellML");

    QString cellmlfile_path = text_CELLML_FILE->text();
    int file_len = cellmlfile_path.length();
    std::string std_file = cellmlfile_path.toStdString();
    const char *file_name = std_file.c_str();
    CellML_load_file(const_cast<char *>(file_name),file_len,&nvariables,&ncomponents);
    if (component_list.size() > 0) {    // Need to clear previous component data
        for (icomp = 0; icomp < component_list.size(); icomp++) {
            component = component_list[icomp];
            delete component->gbox;
            free(component->label_list);
            free(component->line_list);
            component->name[0] = NULL;
            component->nvars = 0;
        }
...
/*
    ...
    component->line_list[ivar] = new QLineEdit;
    component->line_list[ivar]->setMaximumWidth(100);
    component->label_list[ivar]->setText("extra_long_variable_name0");
    form->addRow(component->label_list[ivar],component->line_list[ivar]);
    ivar = 1;
    component->label_list[ivar] = new QLabel;
    component->line_list[ivar] = new QLineEdit;
    component->line_list[ivar]->setMaximumWidth(100);
    component->label_list[ivar]->setText("variable_name1");
    form->addRow(component->label_list[ivar],component->line_list[ivar]);
*/
...
void MainWindow:: startRecorderVTK()
{
    ...
    action_start_recording_VTK->setEnabled(false);
    action_stop_recording_VTK->setEnabled(true);
...
//    hour = summaryData[1]*DELTA_T/60;
    ...
    field->setSliceChanged(true);
    if (Global::conc_nc > 0 && field->isConcPlot()) {
...
void MainWindow::on_pushButtonGetCellMLFile_clicked()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Select CellML File"),
                                                     "", tr("Files (*.*)"));
...
void MainWindow::cellmlParameterChanged()
{
    ...
    int icomp = stringList[2].toInt();
    int ivar = stringList[3].toInt();
    QLineEdit *lineEdit = (QLineEdit *)w;
